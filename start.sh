...
if [ -z "$RUNNING" ]; then
  ...
  ./network.sh up createChannel -c $CHANNEL -ca >> "$CERTCHAIN_DIR/certchain.log" 2>&1
  echo "[2/5] Deploying chaincode..."
  ./network.sh deployCC -ccn certchain -ccp "$CERTCHAIN_DIR/chaincode/" -ccl javascript -c $CHANNEL >> "$CERTCHAIN_DIR/certchain.log" 2>&1
  echo "[3/5] Copying connection profile..."
  cp organizations/peerOrganizations/org1.example.com/connection-org1.json "$CERTCHAIN_DIR/config/connection.json"
...
if [[ "$1" == "--ngrok" ]]; then
  ...
  nohup ngrok http 3000 > "$CERTCHAIN_DIR/.ngrok.log" 2>&1 &
  sleep 4
  ...
  if [ -n "$NGROK_URL" ]; then
    ...
    echo "========================================="
    echo "$NGROK_URL" > "$CERTCHAIN_DIR/.ngrok_url"
  else
