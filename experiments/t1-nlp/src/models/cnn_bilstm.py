"""Baseline: CNN + Bidirectional LSTM."""
...
class CnnBiLstmClassifier(nn.Module):
    """CNN feature extraction followed by Bi-LSTM sequence modeling."""
    ...
    def __init__(self, config: dict):
        ...
        # CNN layers (no pooling — preserve sequence)
    ...
    def forward(self, embeddings: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        conv_outs = [conv(x) for conv in self.convs]  # each (batch, filters, seq_len)
        x = torch.cat(conv_outs, dim=1)  # (batch, filters*n, seq_len)
